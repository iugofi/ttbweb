    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 400) {
                            ...
                            $('#checkotp').val('submit');
                            $('#email, #first_name, #last_name').prop('disabled', true);
                            $('#checkotp').hide();
                            $('#hs-vertically-centered-modal').removeClass('hidden').addClass(
                                'open').css('background', '#1f1f1f47');
                            var email = getEmailFromFormData(formData);
                            $('#emailDisplay').text(email);
                            $('#emailDisplayVal, #emailDisplaypay, #emailDisplayCard').val(
                                email);
                        }
                    }
                });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 400) {
                            ...
                            $('#checkotpinput').val('Verify OTP');
                            $('#hs-vertically-centered-modal').removeClass('open').addClass(
                                'hidden');
                            $('#payment').show();
                        }
                    }
            ...
            function getEmailFromFormData(formData) {
                return decodeURIComponent(formData).split('&').find(function(element) {
                    return element.includes('email=');
                }).split('=')[1];
            }

            // Email field blur event for checking existing user
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.first_name && response.last_name) {
                                ...
                                $('#first_name, #last_name').val('').prop('disabled', false);
                                $('#hidden_first_name, #hidden_last_name').val('');
                            }
                        },
                        error: function() {
                            $('#first_name, #last_name').val('').prop('disabled', false);
